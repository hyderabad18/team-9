...
$result = mysql_query("SELECT title,status,bookid FROM books",$conn);
    ...
    if($check==1){
     ...
     if($check==0){
        ...
        if($flag==0){
            $sql1 = "UPDATE books  SET  status='reserved'  where bookid='$res' ";
            mysql_query($sql1,$conn);
            echo "<script language='javascript'>confirm('This book is successfully reserved for you . ')</script>";    
            echo $res;
        }
        if($flag==1){
            $sql="INSERT INTO `request` (`userid`, `title`, `date`) VALUES ('$userid', '$titles', '$d')";
            echo "Inserted";
...
<head>
 <title>Request</title>
     <link rel="stylesheet" type="text/css" href="modelclass.css">
